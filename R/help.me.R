...
fwrite <- bquote(paste(bold("help.me(\"write\")"), "  Write the contents of mydata to a data file in csv format."))
...
fmean <- bquote(paste(bold("help.me(\"two.samples\")"), "  Compare groups with the mean difference."))
faov <- bquote(paste(bold("help.me(\"many.samples\")"), "  The mean difference and related statistics."))
...
t1 <-
"The example here is for the contributed package smd.and.more. Install one time 
...
randomly sampled value in the lower tail of the respective distribution. That is, 
...
It is convenient to store this generated data for further analysis.  Here, generate 
100 values from a normal distribution with a mean of 50 and a standard deviation 
of 10, store in the vector Y, and then display the resulting histogram.

    > Y <- rnorm(100, mean=50, sd=10)
    > hist(Y)
    
One application of the binomial distribution is to emulate a binary outcome with 
many different trials, such as flipping a coin.  In this example, flip a fair 
coin 20 times with a probability of a Head at 0.5.  Then repeat this set of 20 flips
10 times to get the number of Heads obtained on each set of 20 flips.
    > rbinom(10, 20, .5)"
...
Here is an example of a default histogram.
    > hist(Y)
...
When specifying bins, best to have the stopping point at the end of the last bin, 
such as from the above example with 60, 70, 80, 90, 100. For example, if the data 
ranges from 60 to 100, someone might try this.
    > hist(Y, breaks=seq(60,100,9))
Unfortunately, this sequence yields bins of width 9 that start at 60 and end at 96. 
Values of Y between 96 and 100 would not be included in the histogram, which 
would generate an error message that \"some 'x' not counted ...\"."
...
f3 <- bquote(paste(bold(curve), "  Specifically plot the values of a function."))
...
scatter plots and the values of a function. The sole purpose of the function, curve,
is to plot the values of a function.

This example is the default scatterplot, in color, for variables named X and Y.
    > color.plot(X,Y)

Here is a run chart, in color, for a variable named Y, with an added centerline.
...
f1 <- bquote(paste(bold("length"), "  sample size"))
f2 <- bquote(paste(bold("mean"), "  mean"))
f3 <- bquote(paste(bold("sd"), "  median"))
...
the group means with a t-test.  Here the numerical response variable is named Y 
and the grouping variable, or factor, is X, with exactly two values.
    >  smd.t.test(Y ~ X)
When the tilde, ~, is used to show the relationship between two or more variables, 
R refers to this expression as a formula.

To do a separate analysis of Y for each group, use the [...] notation to define a 
vector that contains just the Y responses for one group.  In this example, one of 
the values of X is Group1. Do for each group.
    > Y.Group1 <- Y[X==\"Group1\"]
    > hist(Y.Group1)

Sometimes the data for a t-test are arranged so that the responses to Y for each
group already are in separate columns called vectors. This example is for doing 
...
f3 <- bquote(paste(bold("chisquare.test"),  "  ", chi^2, " (chi-square) test based on the null hypothesis of no relation."))

t1 <-
"Calculate the cross-tabulation table from the categorical variables, or factors, 
with the table function, applied here to factors X and Y.
    > table(X,Y)
    
To store the counts for later analysis, assign the output of the table function to 
an R object called something such as mytable. Use the addmargins function to 
display the frequencies, as well as the marginal frequencies for the rows and 
columns, of the cross-tabulation table.
    > mytable <- table(X,Y)
    > addmargins(mytable)

Obtain the chi-square statistic calculated with the assumption of no relation 
between the variables, that is, equal proportions.
    > chisq.test(mytable)
"
...
The output of reg also adds suggestions for further analysis, listing the 
function calls for anova, resid, fitted, rstudent, cooks.distance and predict. 
"
...
t3 <- "smd.and.more"
t4 <- "David W. Gerbing"
t5 <- "School of Business Administation"
