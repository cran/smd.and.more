...
if (is.null(ref)) {
	cat(line, pre, " mydata <- read.csv(file.choose())", line)
	mydata <<- read.csv(file.choose(), ...)   # get global scope for mydata
}
else {
...
capture.to.console(head(mydata, n=3))

cat("\n")
cat(line, pre, " tail(mydata, n=3)", "\n", sep="", line, "\n")
capture.to.console(tail(mydata, n=3))
